...
const Alert = ({ title, message, color }: AlertProps) => <div class={`bg-${color}-50 border-l-4 border-${color}-500 text-${color}-800 p-4 mb-2 relative`} role="alert" x-data="{ open: true }" x-show="open">
...
export const Alerts = ({ session }: { session: Session }) => {
  ...
  return <div class="global-alerts">
    {success && success.map((msg) => <Alert title='Success!' message={msg} color='green' />)}
    {error && error.map((msg) => <Alert title='Error!' message={msg} color='red' />)}
    {info && info.map((msg) => <Alert title='Info!' message={msg} color='blue' />)}
  </div>
}
