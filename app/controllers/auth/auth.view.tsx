...
export const login = (ctx: RequestContext): string => (
  <App title="Login" ctx={ctx}>
    ...
    <div>
      ...
      <a
        ...
        href={ctx.url.alias('show.register')}
      >
        Register
      </a>
    </div>
...
export const register = (
  ctx: RequestContext,
  user: User = new User(),
  validation: ValidationErrorCollection = new ValidationErrorCollection([]),
): string => (
  <App title="Register" ctx={ctx}>
    <h1 class="text-2xl text-gray-800">Login</h1>
    <RegistrationForm
      action={ctx.url.alias('register')}
      user={user}
      csrf={ctx.csrfField()}
    ...
    <div>
      ...
      <a class="underline hover:font-bold" href={ctx.url.alias('show.login')}>
        Login
      </a>
    </div>
  </App>
